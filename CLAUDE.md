...
- **No em-dashes, anywhere.** Use a period, a comma, a colon or parentheses, or
  restructure the sentence. This applies to docs, code comments, commit messages,
  PR bodies and the strings a kid reads. 1,619 of them went out in one pass on
  2026-09-04 and a data test (`no em-dashes anywhere in the repo's own text`)
  stops them coming back one at a time. The HUD's "no value" chip keeps an EN
  dash (`–`), which is a different character doing a different job.
